            ...
            if(p.hasPermission("skypvp.statsreset") || p.hasPermission("skypvp.*")) {
                if(cmd.getName().equalsIgnoreCase("statsreset")) {
                    if(args.length == 0) {
                        ...
                        p.sendMessage(prefix + " §7Deine §eStats §7wurden §aerfolgreich §czurückgesetzt!");
                    }else if(args.length == 1) {
                        Player target = Bukkit.getPlayer(args[0]);
                        OfflinePlayer offtarget = Bukkit.getOfflinePlayer(args[0]);
                        if(target != null) {
                            StatsMethod.statsReset(target);
                            p.sendMessage(prefix + " §7Die §eStats §7von §a" + target.getName() + " §7wurden §aerfolgreich §czurückgesetzt!");
                            target.sendMessage(prefix + " §7Deine §eStats §7wurden §czurückgesetzt!");
                        }else if(offtarget != null) {
                            StatsMethod.statsResetOff(offtarget);
                            p.sendMessage(prefix + " §7Die §eStats §7von §a" + offtarget.getName() + " §7wurden §aerfolgreich §czurückgesetzt!");
                        }else
                            p.sendMessage(prefix + " §cDiser Spieler hat nie das Netzwerk betreten!");
                    }else
                        p.sendMessage(prefix + " §cBitte benutze: §7/statsreset <Name>");
                }else
                    p.sendMessage(prefix + " §cBitte benutze: §7/statsreset <Name>");
            }else
